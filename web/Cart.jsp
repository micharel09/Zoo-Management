...
<html>
    ...
    <head>
        ...
        <!-- loading -->
    <iframe id="loading-iframe" src="components/loading.html" frameborder="0" style="
            border: none;
            position: fixed;
            width: 100%;
            height: 100%;
                    ...
                    <form name="f" action="" method="post">
                        <div class="flex space-x-4 mt-4">
                            <c:forEach items="${requestScope.data}" var="p">
                                ...
                                <c:set var="ticketType" value="${p.type}" /> <!-- Thêm một thuộc tính ticketType -->

                                <article class="rounded-xl bg-white p-3 shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300">
                                    <div class="ticket-wrapper">
                                        <div class="relative flex items-end overflow-hidden rounded-xl">
                                            <img class="h-80 rounded-2xl w-full object-cover" src="${ticketType == 'Child' ? 'img/childticket.jpg' : 'img/adultticket.jpg'}" />
                                        </div>
                                        <div class="mt-4 mb-2 flex justify-between items-center">
                                            ...
                                            <div class="flex items-center gap-4">
                                                ...
                                                <div class="mx-auto ml-6">
                                                    <button class="py-2 px-3 bg-transparent text-green-600 font-semibold border border-green-600 rounded-l-3xl hover:bg-green-600 hover:text-white hover:border-transparent transition ease-in duration-200 transform"
                                                            onclick="changeQuantity('${id}', -1)" type="button">
                                                        -
                                                    </button>
                                                    <input class="text-center w-10" type="text" name="num" data-id="${id}" value="0" onchange="validateNumber(this)" />
                                                    <button class="py-2 px-3 bg-transparent text-green-600 font-semibold border border-green-600 rounded-r-3xl hover:bg-green-600 hover:text-white hover:border-transparent transition ease-in duration-200 transform"
                                                            onclick="changeQuantity('${id}', 1)" type="button">
                                                        +
                                                    </button>
                                                </div>
                                                ...
                                                <script>
                                                    ...
                                                    function changeQuantity(id, delta) {
                                                        ...
                                                        var quantityInput = document.querySelector('[data-id="' + id + '"]');
                                                     ...
                                                     function buyTicket(id) {
                                                        ...
                                                        console.log(`Buying ${m} tickets of ID ${id} (${ticketType} ticket)`);
                                                    }
                                                ...
                                                <!-- end quantity -->
                                                <!-- buy ticket -->
                                                <input type="button" class="py-2 px-4 bg-transparent text-green-600 font-semibold border border-green-600 rounded hover:bg-green-600 hover:text-white hover:border-transparent transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0"
                                                       onclick="buyTicket('${id}')" value="Buy Ticket" />
                                                <!-- end buy -->
                                            </div>
                                        </div>
                                    </div>
...
<!--                            <button class="py-2 px-4 bg-transparent text-green-600 font-semibold border border-green-600 rounded hover:bg-green-600 hover:text-white hover:border-transparent transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0"
                                    onclick="buyAll(event)">Buy All Tickets</button>
                        </div>-->
                    </form>
